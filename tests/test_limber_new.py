import numpy as np
import colibri.limber_new as LL
...
pkmm   = S.load_power_spectra(z = zz, k = kk, power_spectra = pkz)
...
# Cosmic shear
W_shear   = S.load_shear_window_functions  (z       = z_gal,
                                            nz      = nz_gal)
# Intrinsic alignment alone
W_IA      = S.load_IA_window_functions     (z       = z_gal,
                                            nz      = nz_gal,
                                            A_IA    = 1.72,
                                            eta_IA  = -0.41,
                                            beta_IA = 2.17,
...
W_lensing = S.load_lensing_window_functions(z       = z_gal,
                                            nz      = nz_gal,
                                            A_IA    = 1.72,
                                            eta_IA  = -0.41,
                                            beta_IA = 2.17,
...
bias     = (1.+z_mean)**0.5
W_galaxy = S.load_galaxy_clustering_window_functions(z = z_gal, nz = nz_gal, bias = bias)
...
#W_HI     = S.load_HI_window_functions         (z=z_gal,nz=nz_gal,bias=1,Omega_HI=0.000625)
#W_CMB    = S.load_CMB_lensing_window_functions(z=z_gal,nz=nz_gal,z_LSS=1089)
#W_custom = S.load_custom_window_functions     (z=z_gal,window=nz_gal)
...
if fourier:
    ...
    # Single components
    Cl_ss = S.limber_angular_power_spectra(l=ll,window_1=W_shear  ,window_2=W_shear  ,power_spectra=pkmm)
    Cl_sI = S.limber_angular_power_spectra(l=ll,window_1=W_shear  ,window_2=W_IA     ,power_spectra=pkmm) + \
            S.limber_angular_power_spectra(l=ll,window_1=W_IA     ,window_2=W_shear  ,power_spectra=pkmm)
    Cl_II = S.limber_angular_power_spectra(l=ll,window_1=W_IA     ,window_2=W_IA     ,power_spectra=pkmm)
    Cl_LL = S.limber_angular_power_spectra(l=ll,window_1=W_lensing,window_2=W_lensing,power_spectra=pkmm)
    Cl_GL = S.limber_angular_power_spectra(l=ll,window_1=W_galaxy ,window_2=W_lensing,power_spectra=pkmm)
    Cl_GG = S.limber_angular_power_spectra(l=ll,window_1=W_galaxy ,window_2=W_galaxy ,power_spectra=pkmm)
...
# so a for loop with fine-tuned 'order' parameter must be performed)
    ...
    Cl_GG = S.limber_angular_power_spectra(l=ll,window_1=W_galaxy ,window_2=W_galaxy ,power_spectra=pkmm)
    # Orders of transform
    order_LL_plus,order_LL_minus,order_GL,order_GG = 0,4,2,0
    # Correlation functions
    xi_ss_p = S.limber_angular_correlation_functions(theta, ll, Cl_ss, order_LL_plus )
    xi_sI_p = S.limber_angular_correlation_functions(theta, ll, Cl_sI, order_LL_plus )
    xi_II_p = S.limber_angular_correlation_functions(theta, ll, Cl_II, order_LL_plus )
    xi_LL_p = S.limber_angular_correlation_functions(theta, ll, Cl_LL, order_LL_plus )
    xi_ss_m = S.limber_angular_correlation_functions(theta, ll, Cl_ss, order_LL_minus)
    xi_sI_m = S.limber_angular_correlation_functions(theta, ll, Cl_sI, order_LL_minus)
    xi_II_m = S.limber_angular_correlation_functions(theta, ll, Cl_II, order_LL_minus)
    xi_LL_m = S.limber_angular_correlation_functions(theta, ll, Cl_LL, order_LL_minus)
    xi_GL   = S.limber_angular_correlation_functions(theta, ll, Cl_GL, order_GL      )
    xi_GG   = S.limber_angular_correlation_functions(theta, ll, Cl_GG, order_GG      )
...
for i in range(nbins):
    plt.plot(zz, W_lensing[i](zz)*1e5,colors[i],ls='-' ,lw=LW,label='Bin %i' %(i+1))
    plt.plot(zz, W_shear  [i](zz)*1e5,colors[i],ls='--',lw=LW)
    plt.plot(zz, W_IA     [i](zz)*1e5,colors[i],ls=':' ,lw=LW)
    plt.plot(zz, W_galaxy [i](zz)*1e3,colors[i],ls='-.',lw=LW)
    #plt.plot(zz, W_HI     [i](zz)*1e1,colors[i],ls=':' ,lw=LW)
    #plt.plot(zz, W_CMB    [i](zz)*1e1,colors[i],ls=':' ,lw=LW)
    #plt.plot(zz, W_custom [i](zz)*1e1,colors[i],ls=':' ,lw=LW)
