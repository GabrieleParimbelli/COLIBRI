...
class limber():
    ...
    # SPIN FACTOR
    #-----------------------------------------------------------------------------------------
    def spin_factor(self, ell, ess, ell_max = 60):
        # Arrays
        ell = np.array(ell,np.int32)
        ess = int(ess)
        # Initialize result
        result = np.zeros_like(ell,np.float32)
        # Indices
        ell_act = np.where(ell<=ell_max)
        ell_pow = np.where(ell>ell_max)
        # Where ell<max use actual computation
        result[ell_act] = np.sqrt(ss.factorial(ell[ell_act]+ess)/ss.factorial(ell[ell_act]-ess))
        result[ell_pow] = (ell[ell_pow]+0.5)**ess
        return result


    #-----------------------------------------------------------------------------------------
    # FULL-SKY ANGULAR SPECTRA
    #-----------------------------------------------------------------------------------------
    def full_sky_angular_power_spectra(self, l, k, z, windows_1, windows_2, power_spectra_1, power_spectra_2):

        # Check scales, redshifts and multipoles
        #try:                   assert all([isinstance(lll, int) for lll in l]), "Multipoles must be integers"
        #except AssertionError: assert all([isinstance(lll, np.int32) for lll in l]), "Multipoles must be integers"
        z, k, l    = np.atleast_1d(z),np.atleast_1d(k), np.atleast_1d(l)
        nz, nk, nl = len(z), len(k), len(l)
        assert nz>3, "At least 3 redshifts, please"
        assert np.all(np.diff(z)<0.05), "Integration not converged, choose smaller redshift steps"
        k_min = k.min()
        k_max = k.max()
        z_min = z.min()
        z_max = z.max()
        assert np.atleast_1d(l).min() > k_min*self.geometric_factor_f_K(z_min), "Minimum 'l' is too low. Extend power spectra to lower k_min? Use lower z_min for power spectrum?"
        assert np.atleast_1d(l).max() < k_max*self.geometric_factor_f_K(z_max), "Maximum 'l' is too high. Extend power spectra to higher k_max? Use higher z_max for power spectrum?"

        # Check dimensions of arrays
        nbins_1 = len(windows_1)
        nbins_2 = len(windows_2)
        for i in range(nbins_1): assert len(windows_1[i]) == nz, "Window 1, bin %i not in correct format" %(i)
        for i in range(nbins_2): assert len(windows_2[i]) == nz, "Window 2, bin %i not in correct format" %(i)
        assert power_spectra_1.shape == (nz,nk), "Power spectra for window 1 not in correct format"
        assert power_spectra_2.shape == (nz,nk), "Power spectra for window 2 not in correct format"

        """
        # Integrals of window functions
        chiz            = self.geometric_factor_f_K(z)
        Kernel_window_1 = np.zeros((nbins_1,nk,nl))
        Kernel_window_2 = np.zeros((nbins_2,nk,nl))
        for iell in range(nl):
            integrand_1 = (power_spectra_1*np.pi/(2*np.expand_dims(k,0)*np.expand_dims(chiz,1)**3))**0.5
            integrand_2 = (power_spectra_2*np.pi/(2*np.expand_dims(k,0)*np.expand_dims(chiz,1)**3))**0.5
            for ik in range(nk):
                for ibin1 in range(nbins_1):
                    k_tmp, T_tmp = FF.Hankel(chiz, windows_1[ibin1]*integrand_1[:,ik], order = l[iell]+0.5, N = 1024)
                    T_tmp_interp = si.interp1d(k_tmp,T_tmp,'cubic',bounds_error=False,fill_value='extrapolate')
                    Kernel_window_1[ibin1,ik,iell] = T_tmp_interp(k[ik])
                for ibin2 in range(nbins_2):
                    k_tmp, T_tmp = FF.Hankel(chiz, windows_2[ibin1]*integrand_2[:,ik], order = l[iell]+0.5, N = 1024)
                    T_tmp_interp = si.interp1d(k_tmp,T_tmp,'cubic',bounds_error=False,fill_value='extrapolate')
                    Kernel_window_2[ibin2,ik,iell] = T_tmp_interp(k[ik])

        # Integral in k
        Cl = np.zeros((nbins_1,nbins_2,nl))
        for ibin1 in range(nbins_1):
            for ibin2 in range(nbins_2):
                Cl[ibin1,ibin2] = sint.simps(np.expand_dims(k,1)**3*Kernel_window_1[ibin1]*Kernel_window_2[ibin2],x=np.log(k),axis=0)
        return Cl
        """

        ""
        # Integral in 'k' (bin-independent)
        PKZ            = np.transpose((np.expand_dims(power_spectra_1,0)*np.expand_dims(power_spectra_2,1))**0.5*k**3.,axes=(2,0,1))
        chiz           = self.geometric_factor_f_K(z)
        KCHI           = np.outer(k,chiz)
        J_ell_chiz     = np.array([ss.jv(l[iell]+0.5,KCHI)*np.sqrt(np.pi/(2.*KCHI)) for iell in range(nl)])
        J_ell_matrix   = np.expand_dims(J_ell_chiz,2)*np.expand_dims(J_ell_chiz,3)*np.expand_dims(PKZ,0)
        J_ell_integral = sint.simps(J_ell_matrix,x=np.log(k),axis=1)

        # Integral of windows and Cls
        Cl       = np.zeros((nbins_1,nbins_2,nl))
        for ibin in range(nbins_1):
            Cl_window_1_integral = sint.simps(windows_1[ibin]*J_ell_integral,x=chiz,axis=2)
            for jbin in range(nbins_2):
                Cl[ibin,jbin] = sint.simps(windows_2[jbin]*Cl_window_1_integral,x=chiz,axis=1)
        return Cl
        ""
